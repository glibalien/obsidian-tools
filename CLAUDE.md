...
│   ├── research.py      # research (agentic LLM pipeline: extract → search → synthesize; note-based or ad-hoc topic)
...
├── hybrid_search.py     # Semantic + keyword search with RRF, BM25, HyDE, reranking, source diversity
...
- **services/compaction.py**: `compact_tool_messages()` replaces tool results with lightweight stubs between turns. Tool-specific stub builders for find_notes (detects semantic vs vault-scan result shape), read_file, find_links, web_search, get_note_info, summarize_file, research; generic fallback for the rest. Dispatches by tool name resolved from assistant messages.
...
- **bm25_index.py**: In-memory BM25 index (`rank_bm25.BM25Okapi`) built lazily from ChromaDB documents. `_tokenize()` splits/lowercases/strips punctuation/filters stopwords. `query_index(query, n_results, chunk_type)` scores and returns top results with post-score `chunk_type` filtering. `invalidate()` forces rebuild on next query (called by `index_vault.py` after indexing). Thread-safe lazy singleton via `threading.RLock()`.
- **hybrid_search.py**: Semantic (ChromaDB) + keyword (BM25) search merged via RRF. Post-retrieval pipeline: cross-encoder reranking (`BAAI/bge-reranker-v2-m3` via `rerank()` from chroma.py) → source diversity (`_diversify()`, max `MAX_CHUNKS_PER_SOURCE` chunks per file). Private `_semantic_retrieve`/`_keyword_retrieve` do raw retrieval; public functions add rerank+diversify. 4x candidate over-fetch for backfill. Keyword search delegates to `bm25_index.query_index()` for IDF-weighted BM25 scoring. HyDE (Hypothetical Document Embeddings): `_is_question()` detects question-type queries, `_generate_hyde()` generates a hypothetical answer via Fireworks LLM, dual semantic search merges original + hypothetical results via RRF. Falls back to standard search on generation failure. Returns `heading` metadata.
- **chunking.py**: Structure-aware chunking (headings → paragraphs → sentences). `_split_by_headings` tracks heading nesting via a level stack, returning `heading_chain` lists for `[Note > Section > Subsection]` prefixes. `_chunk_sentences` carries forward the last `OVERLAP_SENTENCES` (2) sentences between chunks for continuity. `chunk_markdown` applies cross-section overlap (trailing sentences from previous section prepended to next section's first body chunk, with guards for heading-only sections, oversize, and fragment chunks). Frontmatter indexed as dedicated chunk with wikilink brackets stripped. `_parse_frontmatter`/`_strip_frontmatter` also live here.
- **index_vault.py**: Indexing orchestration. Batch upserts per file. `_prepare_file_chunks` builds `[Note > Section > Subsection]` document prefixes from `heading_chain` for richer embedding context. Incremental indexing uses scan-start time. `--full` for full reindex; `--reset` deletes the database and rebuilds from scratch (needed when HNSW index is corrupt, cross-platform-incompatible, or document text format changes). Parallel file reading/chunking via `ThreadPoolExecutor` (`INDEX_WORKERS`) with `_prepare_file_chunks` (pure Python, thread-safe); all ChromaDB operations run on the main thread (ChromaDB is not thread-safe). Failures skip `mark_run` so next run retries; `FileNotFoundError` removes source from `valid_sources` so pruning cleans up.
- **services/chroma.py**: Lazy singletons with `threading.RLock()` for thread-safe init. `embed_documents()`/`embed_query()` apply model-specific prefixes (e.g. nomic `search_document:`/`search_query:`) for embedding computation only — callers store unprefixed text and pass pre-computed embeddings via `embeddings=`/`query_embeddings=`. Auto-detects CUDA for GPU-accelerated embeddings (`_cuda_available()`); ChromaDB's wrapper defaults to CPU. `_check_model_marker()` validates `.embedding_model` file in CHROMA_PATH (legacy DB detection via `chroma.sqlite3` existence). `get_reranker()` lazy singleton for cross-encoder model (`RERANK_MODEL`), `rerank(query, results)` scores+sorts by cross-encoder score (graceful fallback on load failure via `_reranker_failed` flag). `purge_database()` for full DB wipe. Monkey-patches ChromaDB's Posthog `capture()` to no-op (thread-unsafe race in `batched_events` dict). `reset()` for tests.
...
| `batch_move_files` | Move multiple files | `moves` (list of {source, destination}) OR `target_field`/`target_value`/`target_filters` + `destination_folder` (query-based) OR `folder` + `destination_folder`, `confirm` |
...
| `research` | Research topics in a note or ad-hoc topic | `path` OR `topic` (mutually exclusive), `depth` ("shallow"/"deep"), `focus` |
