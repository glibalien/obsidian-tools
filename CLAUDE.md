...
├── hybrid_search.py     # Semantic + keyword search with RRF, cross-encoder reranking, source diversity
...
- **hybrid_search.py**: Semantic (ChromaDB) + keyword search merged via RRF. Post-retrieval pipeline: cross-encoder reranking (`BAAI/bge-reranker-v2-m3` via `rerank()` from chroma.py) → source diversity (`_diversify()`, max `MAX_CHUNKS_PER_SOURCE` chunks per file). Private `_semantic_retrieve`/`_keyword_retrieve` do raw retrieval; public functions add rerank+diversify. 4x candidate over-fetch for backfill. Keyword: single `$or` query, term frequency ranking, `_case_variants()` for case-insensitive matching. Returns `heading` metadata.
