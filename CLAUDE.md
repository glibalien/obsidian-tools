...
| `read_file` | Read any vault file (text, audio, image, Office) | `path`, `offset` (0), `length` (3500). Auto-dispatches by extension: audio→Whisper, image→vision model, .docx/.xlsx/.pptx→text extraction. Markdown files auto-expand `![[...]]` embeds inline (1 level deep, binary results cached by mtime). |
